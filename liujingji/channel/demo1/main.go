...
/**
  ...
  5、元素值从外借进入通道时会被复制即进入的是操作符右边的那个元素值的副本而不是它本身
...
A:
1、对于同一个通道，发送操作直接是互斥的，接收操作之间也是互斥的
...
A:
1、缓冲通道已满，对它的所有操作都会阻塞，直到通道中有元素被接收走。通道会依次通知因阻塞而等待的发送操作所在的goroutine, 依次继续执行发送操作。
2、缓冲通道已空，对它的所有接收超时都会被阻塞，直到通道中有新的元素值出现。通道会依次通知因阻塞而等待的接收操作所在的goroutine，依次继续执行接收操作
